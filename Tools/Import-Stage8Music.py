#!/usr/bin/env python3
"""Import Stage 8's sixteen Avex race cues without decoding their MS ADPCM.
...
TITLE_SOURCE = "https://initiald.sega.jp/inid8/sp_music.html"
...
SONGS = (
    ("avex_01_breakin_out", "Breakin' Out", "Ace"),
    ("avex_02_notings_gonna_stop_us_tonight", "Nothing's Gonna Stop Us Tonight", "Annalise"),
    ("avex_03_come_on_baby", "Come On Baby", "Fastway"),
    ("avex_04_sunlight", "Sunlight", "Kaioh"),
    ("avex_05_prayer", "Prayer", "Ducky Chix"),
    ("avex_06_your_love_is_like_a_medicine", "Your Love Is Like a Medicine", "Mega NRG Man"),
    ("avex_07_when_the_sun_goes_down", "When the Sun Goes Down", "Ken Blast"),
    ("avex_08_super_driver", "Super Driver", "Daniel"),
    ("avex_09_kiss", "Kiss", "Bamboo Bimbo"),
    ("avex_10_far_from_the_light", "Far from the Light", "Leo River"),
    ("avex_11_the_race_of_the_night", "The Race of the Night", "Dave Rodgers"),
    ("avex_12_nonsense_sensation", "Nonsense Sensation", "Paul Harris"),
    ("avex_13_hearts_on_fire", "Heart's on Fire", "David Dima"),
    ("avex_14_adrenaline", "Adrenaline", "Ace"),
    ("avex_15_never_say_never", "Never Say Never", "Manuel"),
    ("avex_16_i_just_wanna_stay_with_you", "I Just Wanna Stay with You", "Dream Fighters"),
)
EXCLUDED_CUES = ("avex_17_burn_inside", "avex_18_outsoar_the_rainbow",
                 "avex_19_raise_up", "avex_20_raise_up_ed")
...
def atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, temporary = tempfile.mkstemp(prefix=".stage8-", suffix=".tmp", dir=path.parent)
    try:
...
def parse_cues(data: bytes) -> list[dict]:
    ...
    require(simple == 11302 and complex_count == 0 and data[27] == 1,
            "Expected Stage 8's 11302 simple cue descriptors and one wave bank")
    ...
    require(data[74:138].split(b"\0",1)[0] == b"IniD8"
            and 0 <= offsets[6] <= len(data)-64
            and data[offsets[6]:offsets[6]+64].split(b"\0",1)[0] == b"IniD8",
...
def parse_wave_bank(path: Path) -> list[dict]:
    ...
    with path.open("rb") as stream:
        ...
        require(flags == 0x80001 and count == 11302 and stride == 24 and name_stride == 64
                and alignment == 2048 and bank[8:72].split(b"\0",1)[0] == b"IniD8",
                "Unsupported Stage 8 wave-bank layout")
...
def prepare(xsb: Path,xwb: Path) -> tuple[list,dict]:
    ...
    require({cue["code"] for cue in cues if cue["code"].startswith("avex_")}
            ...
            "Stage 8 Avex cue roster differs")
    prepared=[]
    with xwb.open("rb") as stream:
        for code,title,artist in SONGS:
            ...
            require(cue["loopCount"] == 255 and wave["channels"] == 2 and wave["blockBytes"] == 44
                    and wave["samplesPerBlock"] == 32, "Unexpected Stage 8 race cue format")
    ...
    provenance=dict(sourceSoundBank="IniD8.xsb",sourceSoundBankSha256=digest(sound_data),
                    sourceWaveBank="IniD8.xwb",sourceWaveBankSha256=bank_hash,
                    sourceCueCount=len(cues),sourceWaveCount=len(waves),formatSources=FORMAT_SOURCES,
                    excludedCueNames=list(EXCLUDED_CUES),
                    excludedPreviewCueNames=sorted(cue["code"] for cue in cues
                                                   if cue["code"].startswith("select_avex_")),
...
def run(project: Path,xsb: Path,xwb: Path,check: bool,extract_only: bool) -> dict:
    ...
    for code,_,_,_,_,data,_ in prepared:
        path=streams/"stage8"/(code+".wav")
        require(not path.exists() or path.read_bytes() == data, "Refusing to overwrite differing Stage 8 asset: "+str(path))
        require(not check or path.exists(), "Stage 8 asset missing: "+str(path))
    if not check:
        for code,_,_,_,_,data,_ in prepared:
            atomic_write(streams/"stage8"/(code+".wav"),data)
    records=[];title_records=[]
    for ordinal,(code,title,artist,cue,wave,_,metadata) in enumerate(prepared):
        identity="stage8."+code
        records.append(dict(id=identity,sourceStage=8,code=code,filename=code+".wav",
                            path="data/original_audio/streams/stage8/"+code+".wav",
                            ...
                            sourceArchive="IniD8.xwb",sourceArchiveByteOffset=wave["sourceByteOffset"],
        ...
        title_records.append(dict(id=identity,title=title,artist=artist,sources=[TITLE_SOURCE],
            fileMappingEvidence=f"IniD8.xsb cue {cue['cueIndex']} named {code}, sound record at {cue['soundOffset']}, "
            f"contains PLAYWAVE index {cue['waveIndex']} into IniD8.xwb. Exact encoded data starts at "
            f"{wave['sourceByteOffset']}; only a RIFF container was added. Display title and artist "
            "come from Sega's original Stage 8 SOUND LIST; native cue spelling remains in the stable ID."))
    if not extract_only:
        ...
        require(len(entries) in (86,102), "Expected 86 existing or 102 already-imported songs")
        old=json.loads((streams/"music_catalog.json").read_bytes())
        require(len(old["tracks"]) in (86,102), "Unexpected existing catalog size")
        for entry in old["tracks"][:86]:
        ...
        for (_,_,_,_,_,data,metadata) in prepared:
            require(exporter.inspect_msadpcm_wav(data,"Stage8") == metadata, "Exporter and XWB metadata disagree")
        ...
        preserved=[record for record in manifest["tracks"] if record["sourceStage"] != 8]
        preserved_titles=[record for record in titles["tracks"] if not record["id"].startswith("stage8.")]
        require(len(preserved) == 73 and len(preserved_titles) == 86, "Previous music metadata roster differs")
        manifest["tracks"]=preserved+records;manifest["trackCount"]=89
        manifest["stage8Import"]=provenance
        manifest["transformation"]="Stages1-5 native files unchanged; Stages6-8 MS ADPCM payloads unchanged inside RIFF/WAVE wrappers"
        titles["tracks"]=preserved_titles+title_records
        new_header=header
        if len(entries) == 86:
            declaration=exporter.DECLARATION.search(header);at=declaration.end(2)
            lines="".join("    {"+", ".join((json.dumps("stage8."+code),json.dumps(title),"8",
                         json.dumps("stage8/"+code+".wav"),json.dumps(artist)))+"},\n" for code,title,artist in SONGS)
            new_header=header[:at]+lines+header[at:]
            import re
            new_header=re.sub(r"(std::array\s*<\s*RaceMusicTrack\s*,\s*)86(\s*>)",r"\g<1>102\2",new_header)
        final_entries=exporter.parse_entries(new_header)
        require(final_entries[:86] == entries[:86]
                and [entry["id"] for entry in final_entries[86:]] == [record["id"] for record in records],
                "Append-only native music order changed")
        for entry,(_,title,artist) in zip(final_entries[86:],SONGS):
            require(entry["title"] == title and entry["artist"] == artist and entry["sourceStage"] == 8,
                    "Stage 8 native metadata differs")
        ...
        require(result["tracks"][:86] == old["tracks"][:86], "Previously exported music records changed")
    ...
    return dict(status="verified" if check else "extracted" if extract_only else "imported",trackCount=102,
                stage8Count=16,previous86CatalogRecordsUnchanged=not extract_only,
                ...
                provenance=provenance,tracks=[dict(index=86+i,**record) for i,record in enumerate(records)])
...
if __name__ == "__main__":
    try:main()
        ...
        raise SystemExit(f"Stage 8 import failed: {error}") from error
