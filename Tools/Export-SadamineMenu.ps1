...
$g.DrawImage($source,[Drawing.Rectangle]::new(0,0,640,360),0,0,1280,720,[Drawing.GraphicsUnit]::Pixel)
$shade=[Drawing.SolidBrush]::new([Drawing.Color]::FromArgb(35,0,0,0));$g.FillRectangle($shade,0,0,640,312)
TextPath $g 'Sadamine' 68 16 32 ([Drawing.Color]::Black) ([Drawing.Color]::White) 4
...
for($i=0;$i -lt $count;$i++){ $x=$r.ReadSingle();$null=$r.ReadSingle();$z=$r.ReadSingle();if($i -ge 95 -and $i -le 3501 -and ($i%8 -eq 7 -or $i -eq 3501)){$points+=,[double[]]@($x,-$z)} };$r.Dispose()
...
$g.Dispose();$card.Save((Join-Path $PreviewDirectory 'sadamine-menu-card.png'))
...
$g.FillRectangle([Drawing.Brushes]::RoyalBlue,0,0,36,14)
$font=[Drawing.Font]::new('Arial',11,[Drawing.FontStyle]::Bold,[Drawing.GraphicsUnit]::Pixel);$g.DrawString('EASY',$font,[Drawing.Brushes]::White,[Drawing.PointF]::new(1,0));$font.Dispose()
TextPath $g 'Sadamine' 19 2 38 ([Drawing.Color]::White) ([Drawing.Color]::Black) 2
$g.Dispose();$tile.Save((Join-Path $PreviewDirectory 'sadamine-menu-tile.png'))
