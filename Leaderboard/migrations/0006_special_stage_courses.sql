-- Widen course directions to 0..29. Copy dependent replay tables before
-- replacing runs so ON DELETE CASCADE cannot remove existing recordings.
CREATE TABLE runs_special_stage (
 id TEXT PRIMARY KEY, device_id TEXT NOT NULL REFERENCES devices(id),
 ruleset TEXT NOT NULL, epoch INTEGER NOT NULL, condition INTEGER NOT NULL CHECK(condition BETWEEN 0 AND 29),
...
INSERT INTO runs_special_stage SELECT * FROM runs;
CREATE TABLE replays_special_stage (
 run_id TEXT PRIMARY KEY REFERENCES runs_special_stage(id),
 data BLOB NOT NULL CHECK(length(data) BETWEEN 44 AND 1010000)
);
INSERT INTO replays_special_stage SELECT * FROM replays;
CREATE TABLE replay_chunks_special_stage (
 run_id TEXT NOT NULL REFERENCES runs_special_stage(id) ON DELETE CASCADE,
 part INTEGER NOT NULL CHECK(part >= 0), data BLOB NOT NULL,
 PRIMARY KEY(run_id, part)
);
INSERT INTO replay_chunks_special_stage SELECT * FROM replay_chunks;
DROP TABLE replay_chunks;
DROP TABLE replays;
DROP TABLE runs;
ALTER TABLE runs_special_stage RENAME TO runs;
ALTER TABLE replays_special_stage RENAME TO replays;
ALTER TABLE replay_chunks_special_stage RENAME TO replay_chunks;
