...
test('Special Stage migration retains records, legacy and chunked replay bytes with foreign keys enabled',()=>{
 const db=new DatabaseSync(':memory:');db.exec('PRAGMA foreign_keys=ON');
 for(const name of ['0001_leaderboard','0002_imported_times','0003_replays','0004_replay_chunks','0005_enna_skyline'])db.exec(readFileSync(new URL('../migrations/'+name+'.sql',import.meta.url),'utf8'));
 ...
 db.exec('BEGIN');db.exec(readFileSync(new URL('../migrations/0006_special_stage_courses.sql',import.meta.url),'utf8'));db.exec('COMMIT');
 for(const table of Object.keys(before))assert.deepEqual(db.prepare('SELECT * FROM '+table).all(),before[table]);
 assert.deepEqual(db.prepare('PRAGMA foreign_key_check').all(),[]);
 db.exec('UPDATE runs SET condition=29');assert.throws(()=>db.exec('UPDATE runs SET condition=30'));
