...
test('upload build policy accepts only the exact ROM-required release',()=>{
 assert.equal(REQUIRED_CLIENT_BUILD,'0.3.95-community-replays.32');
 assert.equal(supportedBuild(REQUIRED_CLIENT_BUILD),true);
 for(const build of ['0.3.95-community-replays.1','0.3.95-community-replays.28','0.3.95-community-replays.29','0.3.95-community-replays.30','0.3.95-community-replays.31','0.3.95-community-replays.33','0.3.95-community-replays.320','0.3.96-community-replays.32','0.4.0','1.0.0','0.3.95','0.3.95-community-replays.032','0.3.95-Community-replays.32','0.3.95-community-replays.32-extra','0.3.95-community-replays.32 ','replay-smoke','',null,32])assert.equal(supportedBuild(build),false,String(build));
...
test('nonmatching builds are permanently rejected before replay work and without database writes',async()=>{
 ...
 for(const build of ['0.3.90-performance.5','0.3.95-community-replays.1','0.3.95-community-replays.28','0.3.95-community-replays.29','0.3.95-community-replays.30','0.3.95-community-replays.31','0.3.95-community-replays.33','0.3.96-community-replays.1','0.4.0','0.3.95-community-replays.032']){
...
test('configured exact release rejects both sides of its version and is exposed read-only',async()=>{
 ...
 assert.equal(db.prepare('SELECT total_changes() n').get().n,before);
 for(const build of ['0.3.95-community-replays.31','0.3.95-community-replays.33'])assert.equal((await uploadReplay(run({build}))).status,409);
