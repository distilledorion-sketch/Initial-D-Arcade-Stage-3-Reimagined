...
public static class Idas3SponsorChecks {
    ...
    public static void Run(){
        ...
        foreach(string variant in new[]{"","day_wet","night_dry","night_wet"}){
            string root=Path.Combine("RuntimeAssets/SADAMINE",variant);
            ...
            using(var r=new BinaryReader(File.OpenRead(Path.Combine(root,"scene.bin")))){
                ...
                for(int shape=0;shape<count;shape++){
                    ...
                    if(manifest.materials[material].name!="makersign_daydry"){r.BaseStream.Position+=nv*44L+nt*4L;continue;}
                    ...
                    object[] args={v,n,uv,uv2,c,ix,variant=="night_wet"?.25f:0};var tags=(Vector2[])method.Invoke(null,args);if(tags==null)continue;
                    ...
                    for(int i=0;i<nt;i+=3){
                        ...
                        if(axis==0)continue;tagged++;Require(axis==.125f||axis==.375f||axis==.625f||axis==.875f,"Wrong sponsor tile");
                        for(int k=0;k<3;k++){var t=tt[ix[i+k]];Require(t.x>=axis/2-.06251f&&t.x<=axis/2+.06251f&&t.y>=0&&t.y<=.25001f,"Non-logo region tagged");}
                    }
                }
            }
            Debug.Log($"Sponsor check {variant}: {tagged} / {checkedCorners}");Require(tagged>100,"Sponsor panels missing: "+variant);summary+=$"PASS {variant}: {tagged} tagged triangles; {checkedCorners} corners preserve positions/normals/colors/UVs; no neighboring triangle tag leaks.\n";
        }
        File.WriteAllText("Verification/signs-enna-20260922/mesh-checks.txt",summary);Debug.Log(summary);
    }
}
