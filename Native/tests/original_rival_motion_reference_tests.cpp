...
int main(int argc,char** argv)try{
    ...
    for(std::uint32_t condition=0;condition<18;++condition){
        ...
        for(std::uint32_t profile=0;profile<32;++profile)for(std::uint32_t variant=0;variant<16;++variant){
            ...
            for(auto& w:rival.words)w=std::bit_cast<std::uint32_t>((float(random()%10000)-5000.f)*.00001f);for(auto& w:pub.words)w=random();for(auto& w:actor.words)w=random();for(auto& w:player.words)w=random();
            ...
            memory.write32(0x0caa9864,variant&1);
            ...
            if(active){advanceOriginalRivalMotion(rival,pub,path,input,variant&1);instructions+=cpu.run(0x0c15b7a8,0x0c15be50,20000);}
    ...
    std::cout<<"PASS original rival pace/movement: "<<cases<<" cases ("<<fractionalCorrections<<" fractional corrections), "<<checks<<" exact comparisons, "<<instructions<<" original instructions, zero hooks\n";
