...
int main(int argc,char** argv)try{
    if(argc!=4)throw std::invalid_argument("Usage: original_driving_rival_session_reference_tests native_root canonical_image primary_fsca_header");
    ...
    for(std::uint32_t variant=0;variant<4;++variant){
        ...
        OriginalDrivingSelection selection;selection.physics=makeOriginalFreshTimeAttackSelection(
            variant?7:0,variant<2?6+variant:12+variant,variant&1?OriginalWeather::Wet:OriginalWeather::Dry);
        selection.collisionVariant=selection.physics.conditionCode&1;
        OriginalDrivingRivalSetup rival;rival.control=reset==0?1:reset==1?-1:-2;
        rival.enemyId0C9015E0=variant<2?13:26;rival.profileMode0C901648=variant==1?2:0;
        rival.geometryCar0C9015F8=variant==1?29:0;rival.level0C9015D0=11;
        ...
        if(variant==0&&reset==0){rival.position=pose.position;rival.position[0]+=.8f;} // actual contact fixture
        ...
        if(variant>=2){const auto alternate=rivals.loadPath(root/"data/original_rival",selection.physics.conditionCode,true);for(std::size_t i=0;i<alternate.points.size();++i)for(unsigned j=0;j<3;++j)memory.writeFloat(alternateBase+std::uint32_t(i*12+j*4),alternate.points[i][j]);}
        ...
        if(session.rivalInitialized()!=(reset==0)||session.rivalActive()!=(reset==0))throw std::runtime_error("Session rival reset status mismatch");
        //062FA0's race auto-brake clear is part of the existing host reset.
        memory.write32(0x0C9008F4,memory.read32(0x0C9008F4)&~0x2000u);
        for(std::uint32_t tick=0;tick<(reset==0?600u:60u);++tick){
            ...
            const float progress=reset==2?-float(tick*3):0.f;const auto correctionMode=(tick/20)&1u;
            ...
            if(reset==0&&tick==250){
                const auto savedPlayer=session.vehicle().drive.words;const auto savedRival=session.rivalActor().words;
                auto bad=selection;bad.physics.conditionCode=6;bad.collisionVariant=0;bad.rival->enemyId0C9015E0=26;bad.rival->profileMode0C901648=0;
                bool rejected=false;try{session.reset(root,bad,pose.position,pose.angles);}catch(const std::runtime_error&){rejected=true;}
                if(!rejected||session.vehicle().drive.words!=savedPlayer||session.rivalActor().words!=savedRival)throw std::runtime_error("Missing alternate PATH reset altered active session");
                OriginalDrivingSession moved=std::move(session);session=std::move(moved);
            }
        }
    }
    }
    if(!contacts)throw std::runtime_error("Integration fixture did not exercise actual body contact");
    std::cout<<"PASS "<<resets<<" complete159720 setups and "<<frames<<" sequential159920 player/rival frames, "<<contacts<<" contact frames, "<<comparisons<<" bit comparisons, "<<instructions
        <<" original instructions, "<<impactRecords<<" impact records. Both Akina/Tsuchisaka directions; primary/alternate paths, profile31 override and sparse disabled resets. Only platform audio/cue/diagnostic/digital-reset hooks; no driving, rival, pair, query, matrix, math, publication or recovery hooks.\n";
