// Source-selector execution for previously omitted position-list billboards.
...
int main(int argc,char**argv){try{
 if(argc!=9)throw std::runtime_error("image scene-index reverse path-count chunk-count output.json weather list-seeds.txt required");
 ...
 if(scene>=std::size(tables)||reverse>1||pathCount<2||pathCount>20000||chunkCount>4096)throw std::runtime_error("Course capture bound");
 ...
 const bool captureLamps=true;
 struct ListRange{unsigned begin,end;bool existing;};std::vector<ListRange> listRanges;
 {std::ifstream seeds(argv[8]);if(!seeds)throw std::runtime_error("Missing billboard seed list");
  unsigned offset,existing,next=0xd060000;std::string pathName;
  while(seeds>>offset>>existing>>std::quoted(pathName)){
   if(offset<0x400||offset>0x600||offset%4||existing>1)throw std::runtime_error("Position-list field bound");
   std::ifstream in(pathName,std::ios::binary);if(!in)throw std::runtime_error("Missing original position-list "+pathName);
   std::vector<unsigned char> data{std::istreambuf_iterator<char>(in),{}};
   if(data.size()<8||data.size()>0x10000)throw std::runtime_error("Position-list file bound");
   for(unsigned i=0;i<data.size();++i)mem.write8(next+i,data[i]);
   const unsigned count=mem.read32(next);
   if(mem.read32(next+4)!=3||data.size()!=8+12*count||count>4096)throw std::runtime_error("Position-list header/extent mismatch");
   mem.write32(obj+offset,next);listRanges.push_back({next+8,next+unsigned(data.size()),bool(existing)});next+=(unsigned(data.size())+63)&~63u;
  }
  if(!seeds.eof())throw std::runtime_error("Malformed position-list seed manifest");
 }
 ...
 const auto vt=tables[constructorScene];mem.write32(obj,vt);mem.write32(obj+4,vt-40);mem.write32(obj+8,vt-64);
 ...
 const unsigned boundaryAddress=mem.read32(mem.read32(0xc191cfc)+(scene/2)*4);std::vector<unsigned> boundaries;int sentinel=0;
 ...
 RefCpu cpu(mem);Matrix current=identity();std::vector<Matrix> matrices;std::vector<Draw> draws;std::vector<LampDraw> lamps;bool lampTranslation=false,billboard=false;unsigned backgrounds=0,inversions=0,excluded=0,existingDraws=0;bool existingList=false;std::vector<std::pair<unsigned,unsigned>> ownerOrder;bool ownerBeforeBillboard=false,billboardBeforeOwner=false;
 if(scene>=4){for(unsigned i=0;i<((scene==12||scene==13)?6u:((scene==14||scene==15)?2u:4u));++i){const unsigned dynamic=obj+0x8000+i*0x200;mem.write32(obj+0x4e4+i*4,dynamic);mem.write32(dynamic,obj+0x9000);}mem.write32(obj+0x9000+36,pathCountHook+4);cpu.callHooks[pathCountHook+4]=[&](RefCpu&){++excluded;ownerOrder.emplace_back(unsigned(draws.size())+existingDraws,unsigned(lamps.size()));};}
 ...
 auto draw=[&](RefCpu&c){if(c.r[5]>=chunkCount)throw std::runtime_error("Original world chunk outside bank at"+std::to_string(c.pr)+" index"+std::to_string(c.r[5]));
  if(billboard){if(existingList)++existingDraws;else lamps.push_back({unsigned(draws.size())+existingDraws,c.r[5],{current[3],current[7],current[11]}});}
 ...
 cpu.callHooks[0xc1fd060]=[&](RefCpu&c){lampTranslation=false;existingList=false;for(const auto& list:listRanges)if(c.r[4]>=list.begin&&c.r[4]<list.end){lampTranslation=true;existingList=list.existing;break;}translate(mem.readFloat(c.r[4]),mem.readFloat(c.r[4]+4),mem.readFloat(c.r[4]+8));};
 ...
 std::map<std::string,unsigned> staticKnown;std::vector<std::string> staticChoices;std::vector<std::pair<unsigned,unsigned>> staticChanges;unsigned previousStatic=~0u;
 ...
 for(unsigned index=0;index<pathCount;++index){mem.writes=initialWrites;cpu.r.fill(0);cpu.fr.fill(0);cpu.xf.fill(0);cpu.fpscrSz=false;cpu.t=false;cpu.fpul=cpu.mach=cpu.macl=0;
  ...
  draws.clear();lamps.clear();ownerOrder.clear();lampTranslation=billboard=false;matrices.clear();current=identity();backgrounds=inversions=excluded=existingDraws=0;
  for(auto entry:{primary,statics}){cpu.r[4]=obj;cpu.r[15]=stack+0xf000;cpu.pr=stop;try{instructions+=cpu.run(entry,stop,500000);}catch(const std::exception&){std::cerr<<"entry="<<std::hex<<entry<<" pc="<<cpu.pc<<" pr="<<cpu.pr<<" r4="<<cpu.r[4]<<" r0="<<cpu.r[0]<<" r1="<<cpu.r[1]<<" r2="<<cpu.r[2]<<" r3="<<cpu.r[3]<<"\n";throw;}if(!matrices.empty())throw std::runtime_error("Original scene unbalanced matrix stack");}
  if(backgrounds!=1||inversions!=2)throw std::runtime_error("Original background anchoring sequence changed");
  for(const auto& owner:ownerOrder)for(unsigned i=0;i<lamps.size();++i)if(owner.first==lamps[i].before){if(i<owner.second)billboardBeforeOwner=true;else ownerBeforeBillboard=true;}
  std::ostringstream staticKey;staticKey<<std::setprecision(9)<<'[';for(unsigned i=0;i<draws.size();++i){if(i)staticKey<<',';staticKey<<"{\"chunk\":"<<draws[i].chunk<<",\"matrix\":[";for(unsigned j=0;j<16;++j){if(j)staticKey<<',';staticKey<<draws[i].matrix[j];}staticKey<<"]}";}staticKey<<']';
  auto staticText=staticKey.str();auto si=staticKnown.find(staticText);unsigned sc;if(si==staticKnown.end()){sc=unsigned(staticChoices.size());staticKnown.emplace(staticText,sc);staticChoices.push_back(std::move(staticText));}else sc=si->second;
  if(sc!=previousStatic){staticChanges.emplace_back(index,sc);previousStatic=sc;}
  std::ostringstream key;key<<std::setprecision(9)<<'[';for(unsigned i=0;i<lamps.size();++i){if(i)key<<',';const auto& l=lamps[i];key<<"{\"before\":"<<l.before<<",\"chunk\":"<<l.chunk<<",\"position\":["<<l.position[0]<<','<<l.position[1]<<','<<l.position[2]<<"]}";}key<<']';
  auto text=key.str();auto found=known.find(text);unsigned choice;if(found==known.end()){choice=unsigned(choices.size());known.emplace(text,choice);choices.push_back(std::move(text));}else choice=found->second;
 ...
 out<<",\"static_assemblies\":[";for(unsigned i=0;i<staticChoices.size();++i){if(i)out<<',';out<<staticChoices[i];}out<<"],\"static_changes\":[";for(unsigned i=0;i<staticChanges.size();++i){if(i)out<<',';out<<'['<<staticChanges[i].first<<','<<staticChanges[i].second<<']';}out<<"],\"owner_before_billboard_ties\":"<<(ownerBeforeBillboard?"true":"false")<<",\"billboard_before_owner_ties\":"<<(billboardBeforeOwner?"true":"false");
 out<<"}\n";
