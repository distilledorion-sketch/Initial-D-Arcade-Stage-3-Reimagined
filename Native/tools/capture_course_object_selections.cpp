// Source-path-dependent calls to the separately owned course scenery.
...
struct ObjectCall{unsigned owner,before;std::array<unsigned,6> fields;};
...
int main(int argc,char**argv){try{
 ...
 std::vector<ObjectCall> objectCalls;
 ...
 if(scene>=4){for(unsigned i=0;i<((scene==12||scene==13)?6u:((scene==14||scene==15)?2u:4u));++i){const unsigned dynamic=obj+0x8000+i*0x200;mem.write32(obj+0x4e4+i*4,dynamic);mem.write32(dynamic,obj+0x9000);if(scene==12||scene==13)mem.write32(dynamic+164,mem.read32(((constructorScene&1)?0xc2a4a40u:0xc2a470cu)+i*4));}mem.write32(obj+0x9000+36,pathCountHook+4);cpu.callHooks[pathCountHook+4]=[&](RefCpu& c){
    const auto owner=(c.r[4]-(obj+0x8000))/0x200;
    if(owner>=6)throw std::runtime_error("Unexpected original course object owner");
    ObjectCall call;call.owner=owner;call.before=unsigned(draws.size()+lamps.size());
    for(unsigned field=0;field<6;++field)call.fields[field]=mem.read32(c.r[4]+144+4*field);
    objectCalls.push_back(call);++excluded;
 };}
 ...
 cpu.callHooks[0xc042520]=[&](RefCpu&){++excluded;};
 cpu.callHooks[0xc085220]=[&](RefCpu& c){
    const std::array<unsigned,4> values{c.r[5],c.r[6],c.r[7],mem.read32(c.r[15])};
    for(unsigned i=0;i<4;++i)mem.write32(c.r[4]+144+i*4,signed32(values[i])<0?999999u:values[i]);
 };
 std::map<std::string,unsigned> known;std::vector<std::string> choices;std::vector<unsigned> staticCounts;std::vector<std::vector<LampDraw>> lampChoices;std::vector<std::pair<unsigned,unsigned>> changes;unsigned long long instructions=0;unsigned lastChoice=~0u;
 ...
 for(unsigned index=0;index<pathCount;++index){mem.writes=initialWrites;cpu.r.fill(0);cpu.fr.fill(0);cpu.xf.fill(0);cpu.fpscrSz=false;cpu.t=false;cpu.fpul=cpu.mach=cpu.macl=0;
  ...
  draws.clear();objectCalls.clear();lamps.clear();lampTranslation=billboard=false;matrices.clear();current=identity();backgrounds=inversions=excluded=0;
  ...
  std::ostringstream key;key<<'[';
  for(unsigned i=0;i<objectCalls.size();++i){if(i)key<<',';const auto& call=objectCalls[i];key<<"{\"owner\":"<<call.owner<<",\"before\":"<<call.before<<",\"chunk_base\":"<<call.fields[5]<<",\"ranges\":[";
    for(unsigned j=0;j<4;++j){if(j)key<<',';key<<call.fields[j];}key<<"]}";}
  key<<']';
  auto text=key.str();const unsigned staticCount=unsigned(draws.size()+lamps.size());const auto unique=std::to_string(staticCount)+text;auto found=known.find(unique);unsigned choice;if(found==known.end()){choice=unsigned(choices.size());known.emplace(unique,choice);staticCounts.push_back(staticCount);choices.push_back(std::move(text));lampChoices.push_back(lamps);}else{choice=found->second;}
 ...
 for(unsigned i=0;i<boundaries.size();++i){if(i)out<<',';out<<boundaries[i];}out<<"],\"object_selections\":[";for(unsigned i=0;i<choices.size();++i){if(i)out<<',';out<<choices[i];}out<<"],\"object_changes\":[";for(unsigned i=0;i<changes.size();++i){if(i)out<<',';out<<'['<<changes[i].first<<','<<changes[i].second<<']';}out<<"],\"original_instructions\":"<<instructions<<",\"background_chunk\":0,\"background_anchor\":\"camera_xz\",\"excluded_dynamic_helpers_last_sample\":"<<excluded;
 out<<",\"base_static_instance_counts\":[";for(unsigned n=0;n<staticCounts.size();++n){if(n)out<<',';out<<staticCounts[n];}out<<']';
