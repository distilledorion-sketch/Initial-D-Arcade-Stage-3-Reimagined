...
struct ImportedCourse {
    // Hakone uses Myogi (0/1); Sadamine uses Usui (2/3), including wet tables.
    // Imported geometry and presentation remain independent of vehicle handling.
    unsigned handlingCondition(bool reverse)const{return (id==10?2u:0u)+unsigned(reverse);}
    static unsigned courseId(const std::filesystem::path& root){
        std::ifstream f(root/"course.id");unsigned value=9;
        if(f&&(!(f>>value)||(value!=9&&value!=10)))throw std::runtime_error("Invalid imported course identity");
        return value;
    }
    unsigned id=9;
    ...
    static ImportedCourse load(const std::filesystem::path& root){
        ImportedCourse c;c.root=root;c.id=courseId(root);if(c.id==10){c.slug="sadamine";c.name="SADAMINE";}c.source=Course::load(root,c.slug,c.name);
    ...
    std::vector<original::OriginalTimeAttackVisit::MapPage> analysisMaps(const original::OriginalTimeAttackTelemetrySnapshot& trace,bool reverse)const{
        ...
        for(unsigned page=0;page<5;++page){
            unsigned section=reverse?4-page:page-1;
            int first=page?checkpoints[section]:checkpoints[0],last=page?checkpoints[section+1]:checkpoints[4];
            ...
            const auto inSection=[&](std::uint32_t progress){if(!page)return true;unsigned a=reverse?checkpoints[4]-last:first-checkpoints[0];unsigned b=reverse?checkpoints[4]-first:last-checkpoints[0];return progress>=a&&progress<=b;};
    ...
    original::OriginalRaceRuleRow rules(bool reverse)const{
        ...
        r.startIndex=reverse?int(center.size())-1-checkpoints[4]:checkpoints[0];r.goalIndex=checkpoints[4]-checkpoints[0];
        for(int i=0;i<3;i++)r.lapIndices[i]=r.sectionIndices[i]=r.extensionIndices[i]=reverse?checkpoints[4]-checkpoints[3-i]:checkpoints[i+1]-checkpoints[0];
        r.sectionIndices[3]=r.goalIndex;return r;
    }
    original::OriginalStartGridPose spawn(bool reverse)const{
        int i=reverse?checkpoints[4]:checkpoints[0];auto p=source.points[i],d=source.points[i+(reverse?-1:1)]-p;
    ...
    original::OriginalStartGridPose onlineSpawn(bool reverse,unsigned slot)const{
        auto out=spawn(reverse);const int index=reverse?checkpoints[4]:checkpoints[0];
    ...
    original::ImportedDrivingRoad drivingRoad(bool reverse)const{
        ...
        road.path.inclusiveLastIndex=unsigned(center.size())-1;road.collision=collision;return road;
    }
    ...
    void resetRules(original::OriginalRaceRules& owner,bool reverse,original::OriginalRacePoint position)const{
        owner.resetImported(center,left,right,reverse,rules(reverse),times[0],{times[1],times[2],times[3],0,0,0},position);
    }
};
}
