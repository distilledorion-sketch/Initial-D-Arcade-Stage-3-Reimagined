...
void validate(const OriginalResultsState& s){
    if(s.carId>=35||s.condition>=idas3::supportedConditionCount||s.sectionCapacity<2||s.sectionCapacity>4||s.sectionCount>s.sectionCapacity||(s.recordFlags&~0x78000000u))throw std::invalid_argument("Unsupported original Time Attack results state");
